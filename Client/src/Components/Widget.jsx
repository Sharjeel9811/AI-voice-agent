...
const Widget = () => {
    ...
    const loadAgent = async () => {
      try {
        ...
        if (data.agent) {
          ...
          setStatus('ready')
          // Cache this successful load so the widget can still boot offline next time.
          try {
            localStorage.setItem('va_agent_' + userId, JSON.stringify(data.agent))
          } catch (e) { /* localStorage unavailable — offline fallback just won't work this time */ }
        } else {
          setStatus('error')
        }
      } catch (err) {
  ...
  var startListening = function() {
    ...
    recognition.continuous = false     // single utterance, stops itself on silence
    ...
    recognition.onend = function() {
      // Only fall back to idle here if we never got a result (onresult already
      // moves state to 'processing' via sendMessage).
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', margin: 0, padding: 0, background: bg, fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', position: 'relative' }}>
        ...
        <AnimatePresence mode="wait">
          {showLanding ? (
            <motion.div key="landing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }}
              ...
              {/* Voice rings — only animate when something is actually happening */}
              ...
              {/* State label — reflects real state only */}
              <AnimatePresence mode="wait">
              ...
              {/* Sound bars — only move while actually listening */}
      ...
      {/* Listening pill — only shown while genuinely listening */}
      <AnimatePresence>
        {appState === 'listening' && (
      ...
      {/* Input bar — now includes a real mic button */}
      <div style={{
